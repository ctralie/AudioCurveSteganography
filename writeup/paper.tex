...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Procedure{ViterbiTargetReparam}{$\hat{T}$, $\text{SWS}_{\ell} ({M})$, $K$}
    \State $N_T \gets \text{len}(\hat{T})$ \Comment{Number of target points}
    \State $N_M \gets \text{len}(\text{SWS}_{\ell}({M}))$ \Comment{Number of SW-STFT frames}
    \State $S[i, j] \gets 0$ \Comment{$N_T \times N_M$ Cumulative cost matrix}
    ...
    \For{$j = 2:N_M$}
        \For{$t = 1:N_T$}
            ...
            \State $S[t, j] \gets S[t, j] + \sum_{i=1}^d (\hat{T}_i[t] - \text{SWS}_{\ell} ({M_{k_i}})[j])^2$ \Comment{Add on matching cost}
